<script lang="js">
     ...
     async function onSubmit(event) {
          ...
          try {
               const res = await api.userSignupSignupPost({ 
                    "username": username,
                    "password": password
               })
...
<div class="min-h-screen bg-black text-white flex flex-col items-center justify-center">
     <div class="w-full max-w-md px-6">
          ...
          <form class="space-y-4" onsubmit={onSubmit}>
               ...
               {/if}
               <button type="submit" class="w-full py-3 bg-white text-black font-semibold rounded-lg hover:bg-gray-100 transition">Sign Up</button>
          </form>
     </div>
</div>
